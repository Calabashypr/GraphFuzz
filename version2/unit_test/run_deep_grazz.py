...
if __name__ == '__main__':
    ...
    seed = "RN"
    g = None
    file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'model_graph_'+seed)
    file_path = os.path.join(file_path, "model")
    seed_graph_path = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'seed_graph')
    seed_graph_path = os.path.join(seed_graph_path, seed)
    ...
    while True:
        ...
        try:
            ...
            with open(seed_graph_path + '.json', 'w') as json_file:
    ...
    for i in range(10000):
        ...
        try:
            ...
            # model_num += 1
            # with open(file_path + str(model_num) + '_' + seed + '.pkl', 'wb') as file:
            #     pickle.dump(lst_g, file)
            with open(log_path,'a+') as f:
                f.write("fail "+str(i+1)+"\n")
        ...
        if round(coverage, 3) in cover_set:
            with open(log_path,'a+') as f:
                f.write("success "+str(i+1)+"\n")
                with open(file_path + str(i+1) + '_' + seed + '.pkl', 'wb') as file:
        ...
        res_dict['tensorflow'] = res_dict['torch']  # tensorflow bug
